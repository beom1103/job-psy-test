      ...
      try {
        const jobRes = await axios.get(
          `https://inspct.career.go.kr/inspct/api/psycho/value/jobs?no1=${
            rate[rate.length - 1].no
          }&no2=${rate[rate.length - 2].no}`
        );
        await setJobs(jobRes["data"]);

        const majRes = await axios.get(
    ...
    <div className="result">
      <section className="hero is-info is-small">
        <div className="hero-body">
          <h1 className="title">| 직업가치관검사 결과표 |</h1>
      ...
      <div>
        ...
        <p>
          {" "}
          내가 중요하게 생각하는 <span>
            {rate[rate.length - 1].name}
          </span>(와)과 <span>{rate[rate.length - 2].name}</span>을(를)만족시킬
          수 있는 직업은 다음과 같습니다.
        </p>
      </div>
      ...
      <div>
        ...
        <table className="table">
          ...
          <tbody>
            {job.map((jobLevel, idx) => {
              const jobData = (jobs || []).filter((i) => {
                return i?.[2] === idx + 1;
              });
              return (
                <tr>
                  <th style={jobData.length <= 0 ? { display: "none" } : {}}>
                    {jobLevel}
                  </th>
                  <td>
                    {jobData.map((level) => {
      ...
      <div>
        ...
        <table className="table">
          ...
          <tbody>
            {major.map((majorLevel, idx) => {
              const majData = (majors || []).filter((i) => {
                return i?.[2] === idx + 1;
              });
              return (
                <tr>
                  <th style={majData.length <= 0 ? { display: "none" } : {}}>
                    {majorLevel}
                  </th>
